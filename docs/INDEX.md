...
|---|----------|------|----------|--------|------------|-------------|
| D01 | **Complete Database Schema** | `docs/DATABASE-SCHEMA.md` | P0 | ✅ Complete (Review) | S3, ADR-001→010 | 43 tables across 8 clusters (39 base + `org_daily_briefings` M021 + `ai_score_feedback` M022 + `offer_templates`/`offers`/`offer_approvals` M028). Full DDL, RLS (all 4 ops), indexes, triggers, functions, JSONB interfaces, volume estimates. `organizations.default_currency` added M028. |
...
| # | Document | Path | Priority | Status | Depends On | Description |
|---|----------|------|----------|--------|------------|-------------|
| D06 | **Offer Management** | `docs/modules/OFFERS.md` | P1 | ✅ Complete (Review) | D01, D02, D03, D05 | 8-state machine (draft→withdrawn), sequential approval chain with auto-skip (G-022), e-sign integration (stub, Phase 5), offer templates, expiry cron, compensation editor, AI comp suggestion + salary band check + offer letter draft. **Phase 4 build complete (5 waves): 97 offer tests (18 state machine + 14 AI + 16 intent + 34 SA + 15 Inngest) + 44 RLS tests.** |
...
| D24 | **Consolidated Testing Strategy** | `docs/TESTING-STRATEGY.md` | P1 | ✅ Complete (Review) | D04 (ADR-004), D01 | Vitest + Playwright config, golden tenant fixture (2 tenants, 5 roles, full entity graph), MSW mock registry (9 services), ~338-case RLS test matrix (adds offer_templates/offers/offer_approvals from P4), 20 E2E scenarios + 5 failure scenarios, CI parallelization. §5.1 adds Offers module row (P4). |
...
| D29 | **Inngest Function Registry** | `docs/INNGEST-REGISTRY.md` | P0 | ✅ Complete (Review) | D03, D06–D12, D13, D17, D19, D23 | 56 Inngest functions across 10 modules (7+6+7+7+7+4+4+3+4+7). v1.0 scope: 41 functions. **10 shipped** (briefing, embedding, dispatch, send-email, interview-reminder + 5 offer functions from P4-W4). 8 cron schedules. |
