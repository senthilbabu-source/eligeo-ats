...
|---|----------|------|----------|--------|------------|-------------|
| D01 | **Complete Database Schema** | `docs/DATABASE-SCHEMA.md` | P0 | ✅ Complete (Review) | S3, ADR-001→010 | 40 tables across 8 clusters (39 + `org_daily_briefings` from Migration 021). Full DDL, RLS (all 4 ops), indexes, triggers, functions, JSONB interfaces, volume estimates. Post-build audit passed. `ai_usage_logs.action` CHECK updated to include `'daily_briefing'`. |
...
| D24 | **Consolidated Testing Strategy** | `docs/TESTING-STRATEGY.md` | P1 | ✅ Complete (Review) | D04 (ADR-004), D01 | Vitest + Playwright config, golden tenant fixture (2 tenants, 5 roles, full entity graph), MSW mock registry (9 services), ~246-case RLS test matrix (adds `org_daily_briefings`), 18 E2E scenarios + 5 failure scenarios (adds E2E-16/17/18 for dashboard), CI parallelization. §5.1 adds Analytics/Dashboard module row. |
...
| D29 | **Inngest Function Registry** | `docs/INNGEST-REGISTRY.md` | P0 | ✅ Complete (Review) | D03, D06–D12, D13, D17, D19, D23 | 55 Inngest functions across 10 modules (adds `analytics/generate-briefing`). Triggers, retries, concurrency, dead-letter handling. 9 cron schedules. v1.0 scope (40 functions). Global defaults. |
| D30 | **User Story Map** | `docs/USER-STORY-MAP.md` | P1 | ✅ Complete | D27, ADR-011 | 179 user stories across 28 sections (173 + R8–R13 dashboard enhancements). Phase 2.7 all ✅ BUILT. R3 caveat updated: stage snapshot vs passthrough funnel. R4 updated: source quality (Wave 2). R8–R13 added for all dashboard enhancement waves. |
