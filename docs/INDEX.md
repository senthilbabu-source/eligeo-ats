...
| D24 | **Consolidated Testing Strategy** | `docs/TESTING-STRATEGY.md` | P1 | ✅ Complete (Review) | D04 (ADR-004), D01 | Vitest + Playwright config, golden tenant fixture (2 tenants, 5 roles, full entity graph), MSW mock registry (9 services), ~294-case RLS test matrix (adds `email_templates` + `notification_preferences` from Wave F), 20 E2E scenarios + 5 failure scenarios (adds E2E-19/20 for email templates + notifications settings), CI parallelization. §5.1 adds Notifications module row. |
